...
/* ------------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------ */
function makeStudioEnv(keyHex: string, fillHex: string): HTMLCanvasElement {
	const W = 1024;
	const H = 512;
	...
	type Box = { x: number; y: number; r: number; core: string; halo: string };
	const boxes: Box[] = [
...
/* ------------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------ */
function makeCardShadow(): HTMLCanvasElement {
	const W = 512;
	const H = 384;
	const cv = document.createElement('canvas');
	cv.width = W;
...
/* ------------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------ */
function makeEdgeGlow(hex: string): HTMLCanvasElement {
	const W = 512;
	const H = 384;
	const cv = document.createElement('canvas');
	cv.width = W;
...
/* ------------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------ */
function buildRoundedRect(aspectHW: number, cornerRadius: number, segments: number = 8): THREE.ShapeGeometry {
	const w = 1;
	const h = aspectHW;
	const halfW = w / 2;
	const halfH = h / 2;
...
/* ------------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------ */
function restPose(k: number, dx: number, dy: number, dz: number): {
	x: number; y: number; z: number; scale: number; rotZ: number;
} {
	return {
		x: k * dx,
		y: -k * dy,
...
interface CardEntry {
	root: THREE.Group;       // outer parent — placed at the card's stack pose
	pivot: THREE.Group;      // pivot group whose origin sits at the card's BL-interior anchor P =
	                         // (-cardW*0.3, -cardH*0.3) in root-local coords. Rotating pivot.rotation.z
	                         // spins the card CCW around P (used by the flip animation).
	card: THREE.Mesh;        // the screenshot plane (child of pivot, offset -P so its centre
	                         // is restored to root's local origin when pivot rotation is 0).
	shadow: THREE.Mesh;      // slightly larger dark plane behind the card (child of root)
	glow: THREE.Mesh | null; // edge glow plane (top card only; child of root)
	mat: THREE.MeshStandardMaterial;
	geom: THREE.ShapeGeometry;
	// Per-card intrinsic aspect (HEIGHT / WIDTH) sourced from the loaded
	// texture's pixel dimensions. Used both to size the card body
	// (cardSize, cardSize * aspectHW) and to position the pivot/mesh offset.
	aspectHW: number;
	// Logical position in the deck. 0 = front, N-1 = back. Re-shuffled by the
	// flip-cycle animation: when the front card flips to the back, every
	// other card's stackIndex decrements by 1.
	stackIndex: number;
	// Previous stackIndex — held during the cycle so idle cards can lerp
	// smoothly from their previous rest pose to the new one.
	prevStackIndex: number;
	// Flip animation state. -1 ⇒ idle. Otherwise 0..FLIP_TOTAL progress.
	flipPhase: number;
}
...
/* ------------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------ */
function applyCardAspect(c: CardEntry, cardSize: number, cardCornerRadius: number): void {
	...
	if (c.glow) c.glow.geometry = newGeom;
	oldGeom.dispose();
...
export default function StackStage(props: StageProps) {
	const {
		rootRef,
		size,
		input,
	...
	const stackGroupRef = React.useRef<THREE.Group | null>(null);
	const cardsRef = React.useRef<CardEntry[]>([]);
	const shadowTexRef = React.useRef<THREE.CanvasTexture | null>(null);
	const glowTexRef = React.useRef<THREE.CanvasTexture | null>(null);
	const envRTRef = React.useRef<THREE.WebGLRenderTarget | null>(null);
	const pmremRef = React.useRef<THREE.PMREMGenerator | null>(null);
	const keyLightRef = React.useRef<THREE.DirectionalLight | null>(null);
	const fillLightRef = React.useRef<THREE.DirectionalLight | null>(null);
	const yawRef = React.useRef(0);
	const pitchRef = React.useRef(0);
	const screenTexturesRef = React.useRef<(THREE.Texture | null)[]>([]);
	const screenAspectsRef = React.useRef<number[]>([]); // per-slot aspectHW
	...
	const flippingCardRef = React.useRef<number | null>(null); // card index currently flipping
	...
	React.useEffect(() => {
		...
		return () => {
			...
			for (const t of screenTexturesRef.current) t?.dispose();
			screenTexturesRef.current = [];
			for (const c of cardsRef.current) {
				(c.card.material as THREE.Material).dispose();
				(c.shadow.material as THREE.Material).dispose();
				c.glow && (c.glow.material as THREE.Material).dispose();
				c.geom.dispose();
			}
			cardsRef.current = [];
	...
	React.useEffect(() => {
		...
		for (const c of cardsRef.current) {
			if (c.glow) {
				(c.glow.material as THREE.MeshBasicMaterial).map = newTex;
				(c.glow.material as THREE.MeshBasicMaterial).needsUpdate = true;
			}
		}
	}, [edgeGlowColor]);
	...
	React.useEffect(() => {
		...
		const settle = () => {
			settled += 1;
			if (!cancelled && settled >= required) setAssetReady(true);
		};
		if (required === 0) {
			setAssetReady(true);
		}
	...
	React.useEffect(() => {
		...
		if (!stack || !shadowTex || !glowTex) return;
	...
	React.useEffect(() => {
		for (const c of cardsRef.current) {
			...
			(c.shadow.material as THREE.MeshBasicMaterial).opacity = shadowStrength;
			if (c.glow) {
				(c.glow.material as THREE.MeshBasicMaterial).opacity = edgeGlowStrength;
			}
		}
	...
	React.useEffect(() => {
		...
		if (!el) return;
		const onPointerDown = () => {
			if (!flipCycle) return; // honor the toggle
	...
	useHeroAnimationFrame(rootRef, ({ delta }) => {
		...
		const camera = cameraRef.current;
		const stack = stackGroupRef.current;
		if (!renderer || !scene || !camera || !stack) return;
		...
		for (let i = 0; i < N; i++) {
			...
			let posX: number, posY: number, posZ: number, scale: number, rotZ: number;
